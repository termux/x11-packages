...
termux_step_make_install() {
	...
	local l
	for l in libandroid-shmem.so libandroid-spawn.so libfreetype.so libiconv.so libz.so.1 libXext.so libX11.so libXrender.so libXrender.so.1 libXrender.so.1.3.0 libXtst.so libXtst.so.6 libXtst.so.6.1.0 libXi.so libxcb.so libXau.so libXdmcp.so libfreetype.so libfontconfig.so; do
